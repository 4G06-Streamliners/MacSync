...
This section explains how MacSync will be verified and validated: who performs each activity, how the SRS and design documents are reviewed, how implementation and tools support ongoing checks, how the plan itself is audited, and how stakeholder validation is organized. It leads directly into the system-level tests in the next major section and defers detailed per-module unit-test enumerations until the MIS is finalized, as summarized below and in Section ``Unit Test Description.''
...
% TA feedback: removed all instructor \wss{} template prompts (magenta in PDF); replaced with project-specific content and explicit statement that per-module enumerations follow the MIS---``if not yet developed, inform in the doc'' per rubric.
Detailed, module-by-module unit test listings require stable interface names and access programs from the MIS. Until the MIS is approved, this section documents selection philosophy, scope, and traceability expectations; subsections organized by \textbf{MIS module name} will be added in a later revision so each test ID, initial state, input, and output can cite a specific exported routine or service described in that document. This aligns with the \emph{Implementation Verification} discussion earlier in this document, which already states that unit suites expand after detailed design is fixed.

\subsection{Overall philosophy for unit test selection}

Verification of internal components will rely primarily on \textbf{automated} tests. Backend units will use \textbf{Jest} with \textbf{Supertest} or direct service calls against a test database; frontend units will use \textbf{Vitest} with \textbf{React Testing Library}. For each MIS-defined access program (public routine, controller handler, or React hook with stable contract), the team will add:
\begin{itemize}
    \item at least one \textbf{nominal} case matching the MIS pre/post-conditions;
    \item \textbf{boundary and invalid-input} cases derived from type ranges, missing fields, and HTTP error paths; and
    \item where logic is non-trivial, \textbf{branch-focused} cases informed by coverage review (``white box'' rationale documented briefly in the test name or adjacent comment in code).
\end{itemize}
Black-box expectations are taken from the SRS behaviours traced to that module; white-box additions close gaps found during review. Rather than duplicate long input/output tables here while interfaces are still moving, the repository will hold the authoritative tuples, and this document will gain one subsection per MIS module listing test file paths and identifiers when the MIS revision is merged.

\subsection{Unit Testing Scope}

\textbf{In scope:} All MacSync modules implemented by the Streamliners team that appear in the MIS and contain non-trivial logic (authentication/session handling, authorization guards, registration and ticketing services, payment orchestration, waiver storage, notifications, and organizer workflows).

\textbf{Out of scope (not verified by this team):} Behaviour inside third-party SaaS components (e.g., hosted payment network behaviour) beyond the integration surface already covered by SRS-linked tests; operating-system or cloud infrastructure failures; and hardware-only issues.

\textbf{Lower priority until core paths are stable:} Purely presentational UI modules with no branching logic may initially rely on snapshot or smoke tests only, with rationale recorded in GitHub Issues if coverage is deferred.

\subsection{Tests for Functional Requirements}

Most functional verification at unit level will be \textbf{automatic}. Manual walkthroughs remain reserved for cross-cutting UX and NFR validation elsewhere in this plan. If a MIS module is simple enough that inspection or linting is sufficient, the team will note that exception in the traceability table entry for that module rather than inventing redundant tests.

\subsubsection{Planned coverage by module (to be named in MIS)}

After the MIS names concrete modules, this document will include one numbered subsection per module (for example, authentication service, event repository, registration workflow). Each such subsection will summarize: responsibilities per MIS, representative black-box cases tied to SRS FR identifiers, and pointers to \texttt{*.spec.ts} / \texttt{*.test.tsx} files. Until then, no placeholder \texttt{test-id} rows are listed so the PDF does not contain empty instructor shells.

\subsection{Tests for Nonfunctional Requirements}

Most non-functional attributes (latency under load, end-to-end usability) are exercised in \textbf{system tests} earlier in this plan. At unit level, only NFRs that map cleanly to a single component need dedicated cases---for example, ensuring password hashes never appear in logs, or that a rate-limiter rejects excess calls with a deterministic status code. Where unit-level NFR checks would duplicate system metrics without adding diagnostic value, the team will rely on the aggregated NFR tests and record that decision in the traceability mapping.

\subsection{Traceability Between Test Cases and Modules}

Once the MIS is available, the team will maintain a matrix or GitHub Project column linking each MIS module to the set of automated test files that exercise it, and vice versa. Spot checks during each sprint will confirm that every MIS module with logic carries at least the nominal SRS-linked case unless explicitly exempted under ``Unit Testing Scope.'' That artifact satisfies the evidence requirement that all in-scope modules have been considered.
