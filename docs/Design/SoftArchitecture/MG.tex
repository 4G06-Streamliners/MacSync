...
2026-04-06 & 1.1 & TA review: standalone abbreviations note; User Interfaces section now explains each figure; caption typo fixed.\\
\bottomrule
\end{tabularx}
...
% TA feedback: this MG carries its own abbreviation table (same expectation as the MIS); do not defer readers to other documents for definitions.
\noindent
Abbreviations and acronyms used in this Module Guide are defined in the table below.
...
satisfied through the \textit{Sign-Up Module \ref{M3}} and its submodules
(\textit{\mref{M3.1}–\mref{M3.3}}). These modules handle all user registration flows including ticket
purchasing, bus and table sign-ups, RSVPs, and refund management. They interact with the
\textit{Registration Rules Module \ref{M6}} to enforce deadlines, seat capacities, and
...
\textit{Payment Processing Module \ref{M1}} and the \textit{Payment Configuration Module
...
\textit{Sign-Up Module \ref{M3}}. Each submodule provides a tailored workflow for its
respective function, while common logic is centralized through the \textit{Sign-Up
Adapter \mref{M3.1}}. This satisfies the SRS need for flexible and reusable registration
...
\textit{RBAC/FBAC Access Control Module \ref{M2}} and the backend
\textit{Access Control Module \ref{M5}}. These modules collectively enforce role-based and
...
the \textit{User Authorization Module \ref{M8}} in conjunction with other backend modules
(\textit{\ref{M4}–\ref{M7}}). The authorization module manages user authentication and secure session
...
communication} are met by the \textit{Notification Handling Module \ref{M7}}. This module
...
\subsubsection{Payment Processing Module (\ref{M1})}
...
\subsubsection{RBAC/FBAC Access Control Module (\ref{M2})}
...
\subsubsection{Sign-Up Module (\ref{M3})}
...
\subsubsection{User Authorization Module (\ref{M8})}

\begin{description}
\item[Secrets:] Identity verification, session tokens. 
\item[Services:] Authenticates the user, issues and validates session tokens, provides identity context to \ref{M2}.
\item[Implemented By:] \progname{}
...
\subsubsection{Payment Configuration Module (\ref{M4})}

\begin{description}
\item[Secrets:] Supported providers, external/internal services, pricing schemas.
\item[Services:] Stores and provides payment parameters to \ref{M1}. 
\item[Implemented By:] \progname{}
\item[Type of Module:] Library Component
\end{description}

\subsubsection{Access Control Module (\ref{M5})}
...
\subsubsection{Registration Rules Module (\ref{M6})}

\begin{description}
\item[Secrets:] Global event constraints (capacity, deadlines) holds all the default event details/policies.
\item[Services:] Validates sign up operations across all sign up modules. Provides \ref{M3} with core information about events.
\item[Implemented By:] \progname{}
\item[Type of Module:] Abstract Object
\end{description}

\subsubsection{Notification Handling Module (\ref{M7})}

\begin{description}
\item[Secrets:] Integrations (emails/push), notification templating, retry policies.
\item[Services:] Sends confirmations, reminders and status updates triggered by \ref{M1}/\ref{M3}.
\item[Implemented By:] \progname{}
\item[Type of Module:] Library Component.
\end{description}
...
\midrule
F101--F104 & \ref{M3} (Sign-Up Module), \mref{M3.1}-\mref{M3.3} (Sign-Up Submodules), \ref{M6} (Registration Rules Module)\\
F105--F107 & \ref{M1} (Payment Processing Module), \mref{M4} (Payment Configuration Module)\\
F108--F110 & \ref{M3} (Sign-Up Module), \mref{M3.1}-\mref{M3.3} (Sign-Up Submodules), \ref{M6} (Registration Rules Module)\\
F111--F113 & \ref{M2} (RBAC/FBAC Access Control Module), \ref{M5} (Access Control Module)\\
F114--F116 & \ref{M8} (User Authorization Module), \ref{M4}-\ref{M7} (Backend Modules)\\
\bottomrule
\end{tabular}
...
\midrule
\acref{acPaymentGateway} & \ref{M1} (Payment Processing Module), \ref{M4} (Payment Configuration Module)\\
\acref{acSignupTypes} & \ref{M3} (Sign-Up Module), \ref{M6} (Registration Rules Module)\\
\acref{acNotifications} & \ref{M7} (Notification Handling Module)\\
\acref{acPolicyRules} & \ref{M6} (Registration Rules Module), \ref{M3} (Sign-Up Submodules)\\
\acref{acAnalytics} & \ref{M5} (Access Control Module), \ref{M7} (Notification Handling Module)\\
\bottomrule
\end{tabular}
...
% TA feedback: briefly explain each UI figure in prose so readers know what they are looking at and how it relates to the architecture (not figures alone).
\noindent
This section shows representative web UI screens for LEMS. Each figure below is described before it appears: the screenshots illustrate end-user flows that exercise the behaviour-hiding modules defined earlier (sign-up, payment, and access control).

\paragraph{Sign-up and registration UI.}
Figure~\ref{fig:ui-signup} shows a sign-up-oriented view where an authenticated user selects an event registration path (e.g.\ bus, table, or RSVP) and submits structured data. The layout corresponds to the Sign-Up Module family (\ref{M3} and its submodules): the page collects form fields that the client sends to backend registration and rules services after access checks succeed.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Signup.png}
    \caption{Representative sign-up / event registration screen}
    \label{fig:ui-signup}
\end{figure}

\paragraph{Payment UI.}
Figure~\ref{fig:ui-payment} shows the payment experience for purchasing tickets or paying fees. It reflects the Payment Processing and Payment Configuration modules (\ref{M1}, \ref{M4}): the user confirms amount and event context, and the client obtains Stripe-backed payment state (e.g.\ PaymentIntent) through the configured API endpoints described in the MIS.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Payment.png}
    \caption{Representative payment checkout screen}
    \label{fig:ui-payment}
\end{figure}

\paragraph{Access control administration UI.}
Figure~\ref{fig:ui-access} shows an administrator-facing view for managing role- and feature-based permissions. It aligns with the RBAC/FBAC Access Control and Access Control Service modules (\ref{M2}, \ref{M5}): the UI is where policy changes are initiated; the authoritative policy store and permission checks remain in backend services as specified in the MIS.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Access_Control.png}
    \caption{Role-based / feature-based access control administration screen}
    \label{fig:ui-access}
\end{figure}
...
The MacSync platform will integrate with McMaster's identity platform to authenticate users and support \ref{M2}. After logging in via MacID their profile will be linked to key information such as unique identifier (MacID), email address, given name, and affiliation (student, employee). By using the university SSO, we ensure that only valid McMaster students are registered on the platform and reduces authentication overhead.

\section{Timeline} \label{SecTimeline}

A breakdown of the team's task delegation and schedule can be found in the GitHub repository
under the Issues and Projects tabs. The delegation assumes even workload distribution as outlined
in the Development Plan.\\\\
\noindent
All documentation deliverables are to be completed prior to the Final Documentation deadline
in April 2026. All feature-related issues (functional and non-functional) are to be completed prior to
the Final Demonstration and Capstone Expo. \\

\noindent
The following outlines the implementation timeline:

\begin{itemize}
    \item \textbf{Phase 1: Proof-of-Concept Demo (due November 2025)}  
    \begin{itemize}
        \item \textbf{\ref{M4}: Payment Processing Module}
        \begin{itemize}
            \item Payment workflow prototype and API integration testing (Abyan)  
            \item Sandbox transactions with Stripe (Farhan)
        \end{itemize}
        \item \textbf{\ref{M2}: RBAC/FBAC Access Control Module}  
        \begin{itemize}
            \item Role and permission model research (Mahad)  
            \item Prototype admin/attendee login flows (Mahad)
        \end{itemize}
        \item \textbf{\ref{M3}: Sign-Up Module}  
        \begin{itemize}
            \item RSVP and table sign-up prototype (Ali, Prerna)  
            \item Early UI mockups and capacity validation logic (Ali, Prerna)
        \end{itemize}
    \end{itemize}

    \item \textbf{Phase 2: Revision 0 Implementation (due February 2026)}  
    \begin{itemize}
        \item \textbf{\ref{M1}: Payment Processing Module}  
        \begin{itemize}
            \item Finalize secure transaction pipeline and refund logic (Abyan)  
            \item Implement audit trail for financial operations (Farhan)
        \end{itemize}
        \item \textbf{\ref{M2}: RBAC/FBAC Access Control Module}  
        \begin{itemize}
            \item Implement role templates and privilege hierarchy (Mahad)  
            \item Integrate authentication middleware (Mahad, Farhan)
        \end{itemize}
        \item \textbf{\ref{M3}: Sign-Up Module}  
        \begin{itemize}
            \item Full bus/table/RSVP integration (Abyan, Prerna)  
            \item Database schema and validation rules (Prerna)
        \end{itemize}
        \item \textbf{\ref{M5}: Admin Dashboard and Analytics Module}  
        \begin{itemize}
            \item Dashboard UI and analytics metrics (Ali, Abyan)  
            \item Implement sales/capacity charts (Ali)
        \end{itemize}
        \item \textbf{\ref{M6}: Registration Rules Module}  
        \begin{itemize}
            \item Develop configuration logic for deadlines and waitlists (Prerna)  
            \item Link with sign-up validation rules (Mahad)
        \end{itemize}
        \item \textbf{\ref{M7}: Notification Handling Module}  
        \begin{itemize}
            \item Email and push notification service setup (Farhan)  
            \item Implement confirmation and reminder templates (Abyan)
        \end{itemize}
    \end{itemize}

    \item \textbf{Phase 2.5: System Integration with MES Teams (January 2026)}  
    \begin{itemize}
        \item Conduct cross-team integration with the other MES Capstone groups to merge the Payments, Access Control, and Event Creation subsystems.  
        \item Establish shared database schema and API endpoints for unified platform operation.  
        \item Perform integration testing of inter-module communication (auth tokens, event sync, and ticket validation).  
        \item Align with MES technical leads to confirm production hosting configuration and shared environment variables.  
        \item Responsibilities:  
        \begin{itemize}
            \item Farhan - Cross-team liaison and CI/CD pipeline alignment  
            \item Mahad - Backend API merging and shared database migration scripts  
            \item Ali - Access token compatibility and role enforcement validation  
            \item Abyan, Prerna - Front-end integration testing and UI regression checks
        \end{itemize}
    \end{itemize}

    \item \textbf{Phase 3: Full System Testing and Stabilization (due March 2026)}  
    \begin{itemize}
        \item Integrate all modules and connect frontend/backend pipelines (all members)  
        \item Conduct automated unit and integration tests via GitHub Actions (Mahad lead)  
        \item Execute usability and security validation per V\&V Plan (Prerna, Ali)
    \end{itemize}

    \item \textbf{Phase 4: Final Demo and Capstone Expo (due April 2026)}  
    \begin{itemize}
        \item System polishing, documentation updates, and final feature refinement (all members)  
        \item Final presentation video, poster, and Rev 0 submission (Streamliners Team)
    \end{itemize}
\end{itemize}

\noindent
All testing and CI/CD pipelines will be continuously maintained during each phase to ensure
progress toward a stable and verified Rev 0 release.
