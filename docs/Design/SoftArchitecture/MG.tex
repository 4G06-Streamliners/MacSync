...
\begin{description}
\item [M1: Payment Processing Module] Handles the logic and communication between
the frontend and backend services for processing event payments and verifying
transactions.

\item [M2: RBAC/FBAC Access Control Module] Manages user permissions and determines
...
\item [M3: Sign-Up Module] Provides functionality for participants to register for
...
\begin{itemize}
    \item [M3.1:] \textbf{Sign-Up Adapter} Standardizes registration logic and
    offers a unified interface to connect multiple sign-up submodules.
    \item [M3.2:] \textbf{Bus Sign-Up Module} Handles registration for bus
    transportation, including capacity validation and seat allocation.
    \item [M3.3:] \textbf{Table Sign-Up Module} Manages registration and table
    assignments for group events.
    \item [M3.4:] \textbf{RSVP Sign-Up Module} Handles RSVP submissions and
...
\begin{description}
\item [M4: Payment Configuration Module] Stores and manages payment parameters such
as currency, and API integration details for supported payment
methods.

\item [M5: Access Control Module] Implements backend logic for role-based and
feature-based permissions, ensuring consistent authorization across the system.

\item [M6: Registration Rules Module] Defines and enforces sign-up policies,
including event deadlines, capacity limits, and participant prioritization
criteria.

\item [M7: Notification Handling Module] Manages the delivery of notifications to
...
\item [M8: User Authorization Module] Handles user authentication, login, and
