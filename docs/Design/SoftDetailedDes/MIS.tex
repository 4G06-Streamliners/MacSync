...
2026-04-06 & 1.1 & TA review: standalone symbols; sufficiency-for-build note; M1/M3/M5 state clarity; Team~10 PR link; appendix and MG-aligned UI figure guidance (see MG). \\
\bottomrule
\end{tabularx}

~\newpage

% TA feedback: each document must carry its own symbols table (do not defer readers to SRS/MG for acronyms).
...
\begin{tabular}{l l} 
  ...
  FR & Functional requirement identifier (project requirements baseline)\\
  HTTP & Hypertext Transfer Protocol\\
  JSON & JavaScript Object Notation\\
  LEMS & Large Event Management System (this product)\\
  M & Module \\
  MacID & McMaster University digital identity (SSO)\\
  MES & McMaster Engineering Society (client organization)\\
  MG & Module Guide (separate architecture document for the same project)\\
  MIS & Module Interface Specification (this document)\\
  NFR & Non-functional requirement identifier (project requirements baseline)\\
  OAuth & Open Authorization protocol\\
  OS & Operating System \\
  R & Requirement\\
  RBAC & Role-Based Access Control\\
  REST & Representational State Transfer (style of HTTP API)\\
  SC & Scientific Computing \\
  SRS & Software Requirements Specification\\
  SSO & Single Sign-On\\
  SSR & Safety or security requirement identifier (from project hazard analysis)\\
  Stripe & Third-party payment processor used by LEMS\\
  TTL & Time-to-live (cache or token lifetime)\\
...
% TA feedback (EnoughToBuild / DetDesDoc): strengthen evidence that an independent developer could implement every module without contacting the team.
\subsection{Sufficiency for Independent Implementation}

\noindent
Each module section in this document is written so that an implementer with comparable skills but no prior knowledge of the team can code the module from the specification alone.
For every module, the MIS specifies: the module name and intent; \emph{Uses} dependencies; exported access programs with input, output, and exception types; \emph{State Variables} as explicit data structures (not merely behaviour names); environment and configuration assumptions; and access-routine semantics (transitions, outputs, exceptions).
REST endpoint paths, Stripe integration boundaries, and authentication token usage are stated where they affect the contract.
Local helper functions are listed when they clarify non-obvious behaviour.
Together, these elements are intended to satisfy the ``enough to build'' criterion for all listed modules.
...
% TA feedback: requirement identifiers are explained here without pointing readers to another document's symbol list.
identifier & \texttt{SRS-FR\#} & Functional requirement label in the project requirements baseline (same prefix as in the \textit{SRS}) \\
identifier & \texttt{SRS-NFR\#} & Non-functional requirement label in the project requirements baseline (same prefix as in the \textit{SRS}) \\
identifier & \texttt{SSR-\#} & Safety/security requirement label from the \textit{Hazard Analysis} \\
...
% TA feedback: avoid implying that essential definitions live only in another document; the hierarchy is restated here in full.
The following table gives the Level~1 / Level~2 module hierarchy for LEMS (the same decomposition described in the project's Module Guide, reproduced here for completeness).
...
\newpage
\section{MIS of Payment Processing Module (M1)} \label{SecMIS}


\subsection{Module}

\texttt{PaymentProcessingModule}
...
\midrule
\texttt{submitPayment}       & PaymentInfo & ConfirmationResponse & InvalidPaymentData, NetworkError \\
\texttt{validatePaymentData} & PaymentInfo & Boolean              & InvalidPaymentData \\
\texttt{requestRefund}       & RefundRequest & RefundResponse      & InvalidRefundRequest, NetworkError \\
\bottomrule
\end{tabularx}
\end{center}
...
% TA feedback: name persistent aggregates explicitly as data structures (lists, dicts, queues), not as vague ``functions''; align payment/refund records with implementable fields.
\begin{itemize}

    \item \textbf{PaymentRecords}: \texttt{list}$\bigl[\langle \text{paymentID},\ \text{userID},\ \text{amount},\ \text{status},\ \text{timestamp}\rangle\bigr]$\\
    Ordered history of payment outcomes visible to this module (and persisted on the server). Each \texttt{paymentID} is the logical identifier returned by the backend; it may correspond one-to-one with a Stripe PaymentIntent or charge ID depending on deployment. Status values include at least \texttt{pending}, \texttt{succeeded}, and \texttt{failed}.

    \item \textbf{PendingPayments}: \texttt{dict}$\bigl[\text{paymentIntentID} \rightarrow \langle \text{userID},\ \text{eventID},\ \text{amount},\ \text{status}\rangle\bigr]$ with $\text{status} = \texttt{pending}$ for all entries in this map\\
    In-flight PaymentIntents not yet terminal. Keys are Stripe PaymentIntent identifiers while a client session holds a client secret.

    \item \textbf{RefundRequests}: queue implemented as \texttt{list}$\bigl[\langle \text{refundID},\ \text{paymentID},\ \text{reason},\ \text{status}\rangle\bigr]$ (FIFO at the head)\\
    Refund work items submitted through \texttt{requestRefund}. The \texttt{paymentID} ties each refund to a row in \textbf{PaymentRecords} (or to the backend record it mirrors).
...
\section{MIS of Sign-Up Module (M3)} \label{MIS_M3}
...
\subsection{Uses}
\begin{itemize}
    \item Registration Rules (M6)
    \item Payment Configuration (M4)
    \item Access Control (M5)
\end{itemize}
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
\texttt{submitSignup} & userID: Str, eventID: Str, data: JSON & success: bool & SignupError \\
\texttt{getSignupStatus} & userID: Str, eventID: Str & status: JSON & NotFoundError \\
\texttt{cancelSignup} & userID: Str, eventID: Str & success: bool & CancellationError \\
\hline
\end{tabular}
\end{center}
...
% TA feedback: state for sign-up should read as explicit persistent structures (typed dict / record fields), not an ambiguous map keyed by a tuple without interpretation.
\begin{itemize}
    \item \textbf{SignUpRecords}: \texttt{dict}$\bigl[(\text{userID},\ \text{eventID}) \rightarrow \text{JSON payload}\bigr]$\\
    Each value is a signup record (e.g.\ event-specific form payload, bus seat choice, table assignment request, RSVP status) validated before write. The composite key $(\text{userID},\ \text{eventID})$ enforces one active signup document per user per event unless superseded by \texttt{cancelSignup}.
\end{itemize}
...
\begin{itemize}
\item transition: Adds or replaces the entry in \textbf{SignUpRecords} for the given \texttt{userID} and \texttt{eventID}.
...
\begin{itemize}
\item transition: Deletes the $(\texttt{userID},\ \texttt{eventID})$ entry from \textbf{SignUpRecords} when present.
...
\item exception: ValidationError if required fields are missing or have invalid data formats.
\end{itemize}
...
% TA feedback: avoid ``loose'' global policyDocument/policyVersion; tie version to a structured policy store so the MIS style matches other modules.
\begin{itemize}
    \item \texttt{roleAssignments}: \texttt{dict} from \texttt{userID} strings to finite sets of role-name strings\\
    Maps each authenticated user to the set of roles currently assigned (RBAC layer). Feature and capability expansion is derived from \texttt{policyByVersion} below, not stored redundantly in this map.
    \item \texttt{policyByVersion}: \texttt{dict} from $\mathbb{N}$ (version keys) to \texttt{JSON}\\
    Immutable snapshots of the complete policy: roles, features, capabilities, and guard rules. Keys are monotonically increasing version numbers assigned when a policy is committed.
    \item \texttt{currentPolicyVersion}: $\mathbb{N}$\\
    Version key whose snapshot is authoritative for \texttt{getPolicy}, \texttt{checkPermission}, and related routines. Updated only when \texttt{setPolicy} successfully validates and persists a new snapshot (the new key is one greater than the previous maximum version in typical implementations).
\end{itemize}
...
\begin{itemize}
\item transition: Validates \texttt{policy} against \texttt{POLICY\_SCHEMA}, stores it in \texttt{policyByVersion} under a new key, and sets \texttt{currentPolicyVersion} to that key.
...
\item transition: None.
\item input: None.
\item output: Returns \texttt{policyByVersion[currentPolicyVersion]} (the active policy snapshot).
\item exception: None.
\end{itemize}
...
\section{MIS of Registration Rules Module (M6)} \label{MIS_M6}

\subsection{Module}

\texttt{RegistrationRulesModule}
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\midrule
\texttt{validateCapacity}     & EventInfo, UserInfo & Boolean & EventFull, InvalidEvent \\
\texttt{validateDeadline}     & EventInfo, UserInfo & Boolean & DeadlinePassed, InvalidEvent \\
\texttt{validateEligibility}  & EventInfo, UserInfo & Boolean & PermissionDenied \\
\texttt{getEventRules}        & EventID             & EventRules & InvalidEvent \\
\bottomrule
\end{tabularx}
\end{center}
...
\section{MIS of Notification Handling Module (M7)} \label{MIS_M7}

\subsection{Module}

\texttt{NotificationModule}

\subsection{Uses}
\begin{itemize}
    \item User Authorization (M8)
    \item Registration Rules (M6)
    \item Payment Configuration (M4)
\end{itemize}
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
\texttt{SendNotification} & userID: Str, message: Str, notif\_type: Str & success: bool & NotificationError \\
\texttt{getNotificationHist} & userID: Str & Notifications: List[Str] & RetrievalError \\
\texttt{subscribeUser} & userID: Str, subscribed: bool & success: bool & SubscribingError \\
\texttt{UnsubscribeUser} & userID: Str, subscribed: bool & success: bool & UnsubscribingError \\
\hline
\end{tabular}
\end{center}
...
\noindent getNotificationHist(userID):
\begin{itemize}
\item transition: None.
...
\section{MIS of User Authorization Module (M8)} \label{MIS_M8}
...
\begin{itemize}
    ...
    % TA feedback: state session rules in this document instead of pointing readers to other docs for requirements text.
    \item Session expiry rules include, at minimum, a 15-minute idle timeout for administrator sessions and standard attendee session limits agreed with the client; implementers must enforce these limits when issuing and validating tokens.
\end{itemize}
...
% TA feedback: remove template placeholder text; describe what the appendix actually contains.
\noindent
This appendix collects reflective write-ups for the Module Interface Specification milestone.
It records team and individual lessons learned, design trade-offs, and cross-team implementation experience; it is not a normative part of the module contracts in the body of the MIS.

\newpage{}

\section*{Appendix --- Reflection}

% TA feedback: replace boilerplate with a concise purpose statement (template ``will be used to evaluate\ldots'' removed).
\noindent
The questions below are answered by team members as part of the capstone reflection for this deliverable.
...
\section*{Appendix --- Peer Module Implementation Reflection}

  What did you learn by implementing another team's module? Were all the details you needed in the
  ...
  % TA feedback: cite the public pull request for the History Manager delivered to Team 10 (``Five of a Kind'') so the marker can verify the contribution.
  \noindent\textbf{Deliverable link (Team 10 --- History Manager):} \url{https://github.com/OmarHassanAdelhamid/Five-of-a-Kind-capstone-project-/pull/328}\\

  \noindent Our team was tasked with implementing the History Manager (M8) module for Team 10
  ("Five of a Kind"). Through this process, we learned that a well-defined Module
  Interface Specification (MIS) significantly reduces the difficulty and effort
  required to understand a foreign codebase. Specifically, the team included a Finite
  State Machine diagram and explicit logic for stack management (such as clearing
  the redoStack upon a new record change) which made the backend logic more strictly
  defined and easier to program. However, there were some areas where we had to make
  assumptions due to a lack of detail. For example, the MIS did not specify how many
  levels of undo/redo should be supported, so we assumed a reasonable default of 100 levels. \\

  \noindent Regarding the documentation, the MIS provided the majority of the necessary technical
  details, including state variables like historyStack and exception handling for empty
  histories. However, we did require clarification regarding the scope of the implementation.
  The SRS document described the feature as allowing users to "access edit history", which
  implied a frontend component for user interaction. Because the boundary between the
  "Interaction Controller"  and the "History Manager" wasn't immediately clear regarding
  UI responsibilities, we had to email the other team. They clarified that our focus
  should be strictly on the backend logic, while they would handle the frontend interface.
  Once this assumption was clear, no further questions were necessary.\\

  \noindent For having our module implemented by another team, we assigned them the User Authorization module.
  We selected this module because it was highly decoupled and straightforward, requiring
  minimal context regarding the rest of our system's specific architecture. The experience was
  pretty seamless, aside from the other team asking where the module was located within our
  MIS, they required no further guidance and no other questions were asked. This lack of questions
  could be used as an indicator that the module was well-documented and that our choice to assign
  a standalone module was a good one.\\

  \noindent Reflecting on both experiences, an improvement we could make to our documentation would be to explicitly
  define the implementation scope (Frontend vs. Backend) within the module guide or MIS introduction.
  While our logic was clear, the confusion we faced regarding Team 10's "History Manager" with
  whether we were building the UI buttons or just the data structure made it clear that interface
  specifications should explicitly state where the data processing ends and the user interaction begins.
  Adding a "Scope of Implementation" note to our modules would clarify this further.\\
