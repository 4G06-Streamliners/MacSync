...
timestamp & \texttt{Time} & Timestamp on system clock; used for TTL / expiry checks. \\
set & \texttt{set}[T] & Finite unordered set of \(T\) (e.g., \texttt{set[Str]}). \\
dictionary & \texttt{dict}[K \(\to\) V] & Finite map from \(K\) to \(V\) (e.g., \texttt{dict[OperationType \(\to\) URL]}). \\
list & \texttt{list}[T] & Finite ordered list of \(T\) (e.g., \texttt{list[Str]}). \\
object & \texttt{JSON} & JSON object as a (key \(\to\) value) mapping with JSON-serializable values. \\
 ...
 & \texttt{opType} & Operation discriminator used as a map key (e.g., \texttt{``create''}, \texttt{``refund''}). \\
