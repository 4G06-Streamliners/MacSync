...
\item The source for the document
  \begin{todolist}
<<<<<<< HEAD
  \item [\cmark]The source is text-based, like \LaTeX{},
=======
...
\item Overall qualities of documentation
  \begin{todolist}
<<<<<<< HEAD
  \item [\cmark] Scope is clear
  \item [\cmark] Document is abstract (for instance, it does not mention the programming
    language (unless this is a true constraint) or the algorithm
    choices (unless the algorithm is the ``what'' rather than the ``how''))
  \item [\cmark] Output of software is unambiguous
  \item [\cmark] Inputs to software are unambiguous
  \item [\cmark] Document is simple (for instance, there is no need for a table of
    contents)
  \item [\cmark] Stakeholders are clearly identified
  \item [\cmark] Environment is clearly identified
  \item [\cmark] Importance of the problem is convincingly stated
  \item [\cmark] Goals provide selling features of the product
  \item [\cmark] Goals are measurable
=======
  \item Scope is clear
