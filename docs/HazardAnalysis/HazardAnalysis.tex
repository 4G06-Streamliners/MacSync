...
1 & Prerna Prabhu & Address TA rubric feedback, comments and improve readability & 2026-04-06\\
\bottomrule
\end{tabularx}
\end{table}
...
% Feedback/style fix: removed an extraneous trailing apostrophe that could appear as a typo in the compiled PDF.
...
% Feedback clarification: aligned naming with the rest of the document (LEMS) to avoid confusion about what system the boundary refers to.
The LEMS platform system that the hazard analysis will be conducted on consists of:
\begin{enumerate}
...
% TA feedback: added a boundary diagram to make component interactions and what is inside/outside the boundary easier to understand at a glance.
\begin{figure}[H]
    \centering
    ...
    \begin{tabular}{c}
        ...
        \begin{minipage}{0.92\textwidth}
            \centering
            \textbf{LEMS System Boundary}\\[0.6em]
            \fbox{\begin{minipage}{0.85\textwidth}
                \centering
                \textbf{LEMS Application (Web + Mobile)}\\
                Dashboards (User/Admin) \(\mid\) Authentication (RBAC/FBAC) \(\mid\) Sign-ups (RSVP/Bus/Table) \(\mid\) Payment UI/Workflow
            \end{minipage}}\\[0.6em]
            \fbox{\begin{minipage}{0.85\textwidth}
                \centering
                \textbf{PostgreSQL Database}\\
                Users \(\mid\) Events \(\mid\) Registrations \(\mid\) Transactions \(\mid\) Roles/Permissions
            \end{minipage}}\\[0.6em]
            \fbox{\begin{minipage}{0.85\textwidth}
                \centering
                \textbf{Integration Interfaces}\\
                Payment Module \(\rightarrow\) External Payment Gateway \quad \(\mid\) \quad Notification Service \(\rightarrow\) External Email/Notification Provider
            \end{minipage}}
        \end{minipage}}
    \end{tabular}
    \caption{System boundary diagram showing in-scope components and external dependencies.}
    \label{fig:system-boundary}
\end{figure}

% Feedback clarification: aligned naming with the rest of the document (LEMS) and tightened wording to match the boundary diagram.
The system boundary in the case of the LEMS platform includes components necessary to support event operations for the MES. This includes the entire application (web and mobile) along with its subcomponents, the PostgreSQL database, and the integration interfaces to payments and notifications. The payment gateway (e.g., Stripe) and notification provider (e.g., Microsoft services) are controlled by external vendors; however, because the platform depends on them for core workflows (payments, refunds, and user communications), they are treated as critical external dependencies within the system boundary for the purposes of this hazard analysis.
...
\begin{enumerate}
    ...
    % Feedback/style fix: corrected grammar and removed incorrect apostrophe in plurals to improve readability.
    \item Any external APIs hosted by third-party providers (e.g., calendars, OAuth)
\end{enumerate}
% Feedback/style fix: corrected spelling and improved sentence flow to match rubric expectations for clarity and professionalism.
These dependencies are all hosted by third-party services and therefore the development team cannot control their downtime or availability. However, an attempt will be made to mitigate these hazards as much as possible in the case they occur.
...
\begin{subfigure}{\textwidth}
    ...
    % Feedback/style fix: corrected duplicated caption to accurately reflect the figure part number.
